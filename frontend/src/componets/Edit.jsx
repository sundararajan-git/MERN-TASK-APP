...
const Edit = (props) => {
  ...
  const updateHandler = async () => {
    try {
      ...
      let res = await fetch(`http://localhost:4000/api/tasks/${editModal._id}`, {
        method: 'PATCH',
        headers: {
  ...
  return (
    <section className='absolute top-0 left-0 right-0 z-50 bg-gray-400/50 w-full h-full flex justify-center items-center'>
      <div className='bg-white rounded-xl shadow-md w-1/2 h-fit p-2'>
        ...
        <div className='p-4'>
          ...
          <div className="flex flex-col gap-2 mt-2 pb-2">
            ...
            <input type="text" name='description' id='Updescription' className='border border-gray-200 rounded-lg p-2 text-gray-500 focus:ring-green-600 focus:ring-1 focus:border-green-600 outline-none' defaultValue={editModal.description} />
          </div>
